    ...
    for day_index, day in enumerate(DAYS):
        for block in WEEKDAY_SCHOOL_BLOCKS[day]:
            if (day, block) in present:
                continue
            option = catalog.pick_for_slot(block, day_index, title_counts, max_repeat)
    ...
    for block, block_outlines in by_block.items():
        ...
        for day_index, outline in enumerate(block_outlines):
            if title_counts[outline.title] >= max_repeat:
                outline = catalog.replace_overused(outline, title_counts, max_repeat, day_index)
            if (
...
def ensure_minimum_variety(
    ...
    for block, block_outlines in by_block.items():
        ...
        for day_index, outline in enumerate(block_outlines):
            ...
            alternative = catalog.pick_unused(block, used_titles, day_index)
            if not alternative:
                break
...
def finalize_outline(
    ...
    filled = fill_missing_slots(sanitized, week_start, catalog, max_repeat)
    varied = enforce_variety(filled, catalog, max_repeat=max_repeat)
    if cfg.enabled:
